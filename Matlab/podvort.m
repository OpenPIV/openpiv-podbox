function [varargout] = pod(handles, step)
% POD is the Proper Orthogonal Decomposition
% 
...
switch step
%% Vorticity (or other scalar function) modes construction
    case{1} % first time we call the function

        [r,c,N] = size(handles.vort);	% 4-D is the number of files

        len = r*c;

        Uf = zeros(len,N);         % r*c*3 length of the vector

        % small rearrangement
        for i = 1:N
            Uf(1:len,i) = reshape(handles.vort(:,:,i),len,1);
...
if length(phi) == len
%     [umodes,vmodes] = deal(zeros(r,c,m));
        [vortmodes] = deal(zeros(r,c,m));
        for i=1:m
            vortmodes(:,:,i) = reshape(phi(1:len,i),[r c] );
...
%        vel = (umodes.^2 + vmodes.^2).^(0.5); % .+wmodes.^2);             % Velocity vector magnitude
        ...
        % misterious -1
        varargout{1} = vortmodes;
%        varargout{2} = vmodes;
else length(phi) == 2*len
        [umodes,vmodes] = deal(zeros(r,c,m));
        for i=1:m
...
%%
%% CASE 3 - reconstruction of instantaneous maps
...
%% CASE 6 is like case 1 - first call and construction of the C matrix, but
% without the MEAN substraction 
    case{6} % first time we call the function, but different from case 1 it is
        ...
        Uf = zeros(len*2,N);     % r*c*3 length of the vector

        % small rearrangement
        for i = 1:N
            Uf(1:len,i) = reshape(handles.u(:,:,i),len,1);
            Uf(len+1:2*len,i) = reshape(handles.v(:,:,i),len,1);
        end

        % Ensemble average of U:
        %         meanU = mean(U,2);
        % meanU = zeros(2*len,1);
%% NOTE - I manually removed the mean
warning('Mean is assumed zero, fix it manually if it is not true')
